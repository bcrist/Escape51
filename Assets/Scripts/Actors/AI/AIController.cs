...
public class AIController : ActorController {
	...
	protected void AIControllerStart_()
	{
		player = GameObject.FindGameObjectWithTag("Player");
	}
	
	protected void AIControllerUpdate_()
	{
		Vector3 toPlayer = player.transform.position - transform.position;
		float range = toPlayer.magnitude;
		bool seesPlayer = IsAwareOfPlayer(toPlayer, range);
		
		horizontalLookIntention = toPlayer.x;
	...
	private bool IsAwareOfPlayer(Vector3 toPlayer, float range)
	{
